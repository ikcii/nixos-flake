{
  ...
  outputs =
    ...
    let
      ...
      listDirs =
        path: lib.attrNames (lib.filterAttrs (name: type: type == "directory") (builtins.readDir path));
    ...
    in
    {
      ...
      homeConfigurations =
        let
          ...
          specializedConfigs = lib.listToAttrs (
            lib.concatMap (
              system:
              lib.concatMap (
                username:
                  ...
                  if gpuHint != null then
                    {
                      name = username;
                      value =
                        if gpuHint == "rocm" || gpuHint == "cuda" then
                          specializedConfigs."${username}@${builtins.currentSystem}+${gpuHint}"
                        else
                          baseConfig;
                    }
                  else
